...
class Modalpage extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.render === 'profile update') this.setState({ profileUpdate: !this.state.profileUpdate })
    ...
    render() {
        ...
        return (
            <Container>
                <Button onClick={toggle}>{profileUpdate ? 'Edit profile' : 'View Attendees'}</Button>
                <Modal isOpen={modal} toggle={toggle}>
                    <ModalHeader toggle={toggle}>{profileUpdate ? 'Edit profile' : 'View Attendees'}</ModalHeader>
                    <ModalBody>
